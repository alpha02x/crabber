...
export class BlueLootCalculator extends React.Component<BlueLootCalculatorProps, BlueLootCalculatorState> {
	...
	render(): React.ReactNode {
		...
		return (
			<Translation>{t =>
				<div className={(this.props.charsToIncomeMap.size < 1 ? "hidden " : "") + 'col-start-1 col-end-2 max-w-fit shadow-lg pt-1 bg-[#f9fafb] dark:bg-zinc-600 rounded-xl overflow-x-auto overflow-y-visible'}>
					<div className="px-3 my-0.5 dark:text-zinc-100">
						{t("cargo")}:
					</div>
					<textarea
						id='cargoInput'
						...
						placeholder={t('cargo_placeholder')}
					...
					{((document.getElementById("cargoInput") as HTMLInputElement)?.value ?? "") !== "" && (
						<div className='animate-fadein100 mt-2'>
							...
							>
								<p>{isEnoughCargo ?
									t("enough_blue_loot_in_cargo") :
									t("not_enough_blue_loot_in_cargo", {
										missing_isk: (this.props.totalFarmedMoney - this.getCargoPrice(this.state.cargoContents))
							...
							<table className='dark:text-zinc-100'>
								<tbody>
									<tr className='px-3 bg-zinc-200 dark:bg-zinc-400'>
										<th className="px-3 font-normal text-left">
											{t("toon")}
										</th>
										{Array.from(BlueLootDefinitons.keys()).map((blueLootName) => (
											<th key={`bl-header-${blueLootName}`} className='font-semibold text-[12px] max-w-24 px-2 py-0.5 uppercase'>{blueLootName}</th>
										))}
										<th className="px-3 text-right font-normal text-nowrap">
											{t("blue_loot_price")}
										</th>
									</tr>
